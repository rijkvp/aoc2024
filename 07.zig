...
fn canProduceInner(current: u64, i: usize, target: u64, nums: []const u64, comptime allowConcat: bool) bool {
    ...
    return canProduceInner(current * nums[i], i + 1, target, nums, allowConcat) or canProduceInner(current + nums[i], i + 1, target, nums, allowConcat) or (allowConcat and canProduceInner(concat(current, nums[i]), i + 1, target, nums, allowConcat));
}

fn canProduce(target: u64, nums: []const u64, comptime allowConcat: bool) bool {
    return canProduceInner(nums[0] * nums[1], 2, target, nums, allowConcat) or canProduceInner(nums[0] + nums[1], 2, target, nums, allowConcat) or (allowConcat and canProduceInner(concat(nums[0], nums[1]), 2, target, nums, allowConcat));
}
